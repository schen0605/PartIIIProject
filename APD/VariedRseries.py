...
f = open("Data/VariedRseries.txt","r")
...
# plt.title('$\Delta$V = 1V, C$_d$ = 50fF, R$_q$ = 500k$\Omega$, C$_q$ = 20fF')
plt.xlabel('Time [ns]')
plt.ylabel('Current [$\mu$A]')
plt.legend(frameon=False,ncol=2)

a = plt.axes([0.55, 0.4, .3, .3])

for i in range(len(xVar)):
    plt.plot(xVar[i]*1E9, yVar[i]*1E6, linewidth = 2)
plt.xlim(9, 12)
