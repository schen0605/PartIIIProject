...
theoreticalTau = R_q * (capacitance+C_q)
...
plt.plot(capacitance[::2]*1E15, recoveryTime[::2]*1E9,'+', markersize=10)
plt.plot(capacitance*1E15, theoreticalTau*1E9, '--r',linewidth = 2, label = 'Theoretical')

# plt.title('$\Delta$V = 1V, R$_d$ = 1k$\Omega$, R$_q$ = 500k$\Omega$, C$_q$ = 20fF, $\\tau_{theory}$=R$_q$(C$_d$+C$_q$)')

plt.xlabel('C$_d$ [fF]')
plt.ylabel('Recovery Time [ns]')
plt.legend(loc='lower right', frameon=False)

# plt.savefig("Graphics/VariedCdTauFINAL.png")
plt.show()
