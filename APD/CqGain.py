...
for i in range(len(xVar)):
    ...
    capacitance[i] = float(graphLabel[i][4:-1]) * 1E-15

theoretical = ((C_d+capacitance)*V_ov)/ELECTRON_CHARGE
...
params, covariance = optimize.curve_fit(linFunc, capacitance, gain)
print params[0]
...
plt.plot(capacitance*1E15, gain*1E-6, '+', markersize=10)
plt.plot(capacitance*1E15, theoretical*1E-6+0.6, '--r',linewidth = 2, label = 'Theoretical')
